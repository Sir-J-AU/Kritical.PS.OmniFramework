∩╗┐# Kritical.PS.OmniFramework ΓÇö AGENTS.md
...
|---|---|---|---|---|
| `AGENTS.md` | Kritical.PS.OmniFramework ΓÇö AGENTS.md | 2026-08-13 | R1-root-entry | ΓÇö |
| `README.md` | Kritical.PS.OmniFramework ΓÇö Kritical multi-OS PowerShell Foundation | 2026-07-03 | R1-root-entry | ΓÇö |
| `docs/ARCHITECTURE.md` | Kritical.PS.OmniFramework ΓÇö Architecture | 2026-07-03 | R2-design-named | ΓÇö |
...
Invoke-KriticalLensSweep -Root 'C:\Users\joshl\OneDrive - Kritical Pty Ltd\Github\Kritical.PS.OmniFramework'
...
Invoke-KriticalLensLanguageSweep -Root 'C:\Users\joshl\OneDrive - Kritical Pty Ltd\Github\Kritical.PS.OmniFramework'

# both families, plus a combined receipt
Invoke-KriticalLensEstateSweep -Roots 'C:\Users\joshl\OneDrive - Kritical Pty Ltd\Github\Kritical.PS.OmniFramework'

# end-of-turn gate (parser + PsLint auto-fix + PSScriptAnalyzer + Lens + corpus top-up)
& "$lens\scripts\Invoke-KritEndOfTurnGate.ps1" -Repo 'C:\Users\joshl\OneDrive - Kritical Pty Ltd\Github\Kritical.PS.OmniFramework'
```
...
| Raw git corpus (every commit, every blob) | SQL Server `.\SQLEXPRESS` database `KritLens_Kritical_PS_OmniFramework` (tables `lens.git_corpus_version` / `lens.git_corpus_blob`) | `Invoke-KritLensGitCorpusFullIngest` |
...
| --- | --- | --- |
| `Kritical.Lens.PSGraph` | YES (always runs) | 34 PowerShell file(s) ΓÇö PS call graph, HOT/MEDIUM/COLD ranking |
...
| `Language.CMD` | NO ΓÇö no analyzer entry | ingest-only src; no Invoke-KriticalLensLanguageCMD.mjs |
| `Language.CSV` | NO ΓÇö no analyzer entry | ingest-only src; no Invoke-KriticalLensLanguageCSV.mjs |
| `Language.JSON` | NO ΓÇö no analyzer entry | **BLIND SPOT: 1 file(s) in this repo are unanalyzed** |
| `Language.Python` | NO ΓÇö no analyzer entry | ingest-only src; no Invoke-KriticalLensLanguagePython.mjs |
...
| Markdown | 8 |
| PowerShell | 34 |
| **all tracked-ish files (excl. .git/node_modules/bin/obj/dist)** | **49** |
